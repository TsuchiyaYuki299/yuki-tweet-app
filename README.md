...
## ⚠️ 注意事項
このアプリはブラウザの保存領域（LocalStorage）を使用しています。そのため、以下の操作を行うと過去のつぶやきがすべて消えてしまいますのでご注意ください。

- ブラウザの「閲覧履歴データの削除（Cookieとサイトデータ）」を行う

- アプリを開いているブラウザ（ChromeやSafariなど）をアンインストールする

- シークレットモード（プライベートモード）で使用し、タブを閉じる